...
class TrafficGenerator:
    ...
    def traffic_demand(self, route_index, step_counter):
        max_range = 0.1
        statement = step_counter + route_index*(step_counter/10)

        d_i = max_range* (math.sin(statement) + 1) / 2
        ...
        with open("environment/episode_routes.rou.xml", "w") as routes:
            ...
            for step in range(self._max_steps):
                for route in range(1, 5):
                    D_i = self.traffic_demand(route, step)
                    if np.random.choice([True, False], p=[D_i, 1 - D_i]):
                        straight_or_turn = np.random.uniform()
                        if straight_or_turn < 0.75:  # choose direction: straight or turn - 75% of times the car goes straight
                            if route == 1:
                                print('    <vehicle id="W_E_%i" type="standard_car" route="W_E" depart="%s" departLane="random" departSpeed="10" />' % (step, step), file=routes)
                            elif route == 2:
                                print('    <vehicle id="N_S_%i" type="standard_car" route="N_S" depart="%s" departLane="random" departSpeed="10" />' % (step, step), file=routes)
                            elif route == 3:
                                print('    <vehicle id="E_W_%i" type="standard_car" route="E_W" depart="%s" departLane="random" departSpeed="10" />' % (step, step), file=routes)
                            else:
                                print('    <vehicle id="S_N_%i" type="standard_car" route="S_N" depart="%s" departLane="random" departSpeed="10" />' % (step, step), file=routes)
                        else:  # car that turn -25% of the time the car turns
                            if route == 1:
                                print('    <vehicle id="W_S_%i" type="standard_car" route="W_S" depart="%s" departLane="random" departSpeed="10" />' % (step, step), file=routes)
                            elif route == 2:
                                print('    <vehicle id="N_W_%i" type="standard_car" route="N_W" depart="%s" departLane="random" departSpeed="10" />' % (step, step), file=routes)
                            elif route == 3:
                                print('    <vehicle id="E_N_%i" type="standard_car" route="E_N" depart="%s" departLane="random" departSpeed="10" />' % (step, step), file=routes)
                            else:
                                print('    <vehicle id="S_E_%i" type="standard_car" route="S_E" depart="%s" departLane="random" departSpeed="10" />' % (step, step), file=routes)
